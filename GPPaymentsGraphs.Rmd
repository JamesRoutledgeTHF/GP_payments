...
finances_IMD_TOTAL <- finances_IMD_TOTAL %>%
  left_join(
    workforce_imd %>%
      mutate(IMD_QUINTILE = as.factor(IMD_QUINTILE)) %>%
      select(IMD_QUINTILE, YEAR, adj_population = total_population),
...
finance_imd_ADJ <- finances_df %>%
  left_join(prac_imd, by = c("YEAR", "PRACTICE_CODE")) %>%
  mutate(IMD_QUINTILE = as.character(IMD_QUINTILE)) %>%  # make character
  group_by(IMD_QUINTILE, YEAR) %>%
  summarise(
  ...
  left_join(
    workforce_imd %>%
      ...
      select(IMD_QUINTILE, YEAR, total_population),
    by = c("IMD_QUINTILE", "YEAR")
  ) %>%
  rename(TOTAL_ADJ = total_population) %>%
  mutate(
    PAYMENT_PER_ADJ_PATIENT = TOTAL_PAYMENTS / TOTAL_ADJ
  ) %>%
  na.omit()
...
Total_Finance_Categories <- finances_IMD_TOTAL %>%
  mutate(
    Payment_Category = factor(Payment_Category, levels = ordered_levels_sum)
  )

Avg_Finance_Categories <- finances_IMD_TOTAL %>%
  mutate(
    Payment_Category = factor(Payment_Category, levels = ordered_levels_avg)
  )
...
Total_Finance_Categories <- Total_Finance_Categories %>%
  ...
  ungroup() %>%
  mutate(Payment_Category = factor(Payment_Category, levels = ordered_levels_sum))
...
Registered_Patients_Finance_Categories <- Avg_Finance_Categories %>%
  ...
  mutate(Payment_Category = factor(Payment_Category, levels = ordered_levels_avg))
